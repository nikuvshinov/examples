# Сервис интеграций с календарями (service-calendars-integration)

___


*Сервис предназначен для взаимодействия с календарями пользователя: проверка интеграции, просмотр свободных
временных слотов для записи встречи, добавления встречи в календарь, удаление встречи из календаря. Сервис написан на
Python 3.10. С использованием библиотеки fastAPI*
...
- **google calendar** - сервис сервис интегрирован с гугл календарем и взаимодействует с ним с помощью API google (
  библиотеки google-api-python-client google-auth-httplib2 google-auth-oauthlib). Для доступа к календарям требуется
  access token пользователя.

- **cal.com** - сервис сервис интегрирован с cal.com и взаимодействует с ним с помощью API cal.com. Для доступа к календарям требуется
  {username}:{api_key}:{eventTypeId} пользователя.
...
Бизнес логика интеграций с разными календарями содержится в файлах директории [/src/services](/src/services).
...
Для добавления новых интеграций добавляем новый класс календаря в [/src/services](/src/services) (Для удобства можно
наследоваться от [base_calendar.py](/src/services/base_calendar.py)).
Затем добавить роутер для новой интеграции в [/src/routers](/src/routers)
